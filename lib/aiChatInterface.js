...
class AIChatInterface {
  ...
  initializeEventListeners() {
    ...
    // Import Config 按鈕
    this.elements.importConfigBtn.addEventListener('click', () => this.elements.importConfigFile.click());
    this.elements.importConfigFile.addEventListener('change', (e) => this.importConfig(e));

    // 設定變更時儲存
    const settingsElements = [
      this.elements.scriptwriterVersion,
      this.elements.studentVersion,
      this.elements.coachVersion,
    ];
  ...
  updateBotTitles() {
    ...
    this.elements.coachTitle.textContent = coachVersion ? `${coachBaseName} - v${coachVersion}` : coachBaseName;
  }
  ...
  async startScriptwriter() {
    ...
    try {
      ...
      this.elements.createNewStudentBtn.textContent = '取得新的學生角色';
    }
  }
  ...
  clearChat() {
    ...
    this.elements.sidebarContent.innerHTML = `
      <div class="empty-sidebar">
        ...
        <div class="empty-sidebar-subtext">點擊「取得新的學生角色」開始</div>
      </div>
    `;
  ...
  exportConfig() {
    const config = {
      scriptwriter: {
        version: this.elements.scriptwriterVersion.value,
      },
      student: {
        version: this.elements.studentVersion.value,
      },
      coach: {
        version: this.elements.coachVersion.value,
      },
    };
  ...
  importConfig(event) {
    ...
    reader.onload = (e) => {
      try {
        ...
        // 載入配置
        if (config.scriptwriter) {
          this.elements.scriptwriterVersion.value = config.scriptwriter.version || '';
        }

        if (config.student) {
          this.elements.studentVersion.value = config.student.version || '';
        }

        if (config.coach) {
          this.elements.coachVersion.value = config.coach.version || '';
        }
